...
The reproduction uses the real built Nanobrowser extension loaded in Microsoft Edge, a local large-DOM test page, and local Ollama. The Navigator model request is sent to a real local `llama3.2:3b` model through a localhost logging proxy.
...
```powershell
cd E:\agent_vul\nanobrowser
D:\develop\node.js\corepack.cmd pnpm install --frozen-lockfile
D:\develop\node.js\corepack.cmd pnpm build
```

The built unpacked extension is produced at:
...
The final reproduction uses:

```text
Planner: local deterministic OpenAI-compatible mock, only to reach Navigator reliably
Navigator: real local Ollama provider, model llama3.2:3b
Ollama proxy: http://127.0.0.1:11435 -> http://127.0.0.1:11434
```

This is a deliberate compromise: small local models can fail Nanobrowser's Planner structured-output step nondeterministically, so the Planner is mocked. The vulnerable sink under test is the Navigator browser-state model call, and that request is sent to the real local Ollama model.

Safety limits:

```text
Scope: localhost only
Task count: 1 Nanobrowser task
Large model request count: 1 Navigator request
Health probe timeout: 5 seconds
Ollama proxy upstream timeout: 30 seconds
...
# Confirm the chosen model exists locally.
ollama list

C:\Users\guopeng\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe `
  E:\agent_vul\reports\Nanobrowser_Message_Budget_DoS_Report\lab\reproduce_nanobrowser_ollama_dos.py
```

Successful bounded output from this environment:

```text
[baseline] Ollama health before task: {"ok": true, "status": 200, "elapsed_ms": 271, ...}
[planner-mock] request body_bytes=9724 message_chars=9079 max_message_chars=7000
[ollama-proxy] request path=/api/chat body_bytes=1120328 model=llama3.2:3b message_chars=1111739 max_message_chars=1101715
[probe] TIMEOUT: ordinary Ollama health request exceeded 5s while large prompt was active
[ollama-proxy] upstream timeout after 30s path=/api/chat
[recovery] RECOVERED: {"ok": true, "status": 200, "elapsed_ms": 28469, ...}
[SUCCESS] bounded local Ollama reproduction completed
```
...
The proxy log confirms the oversized request reached the real local Ollama provider path:
